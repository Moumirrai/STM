...
testData = create_tie_structure(0.2)

truss: TrussData = parse_structure_data(testData)
...
for eigenstrain in eigenstrainSets:
    truss: TrussData = parse_structure_data(testData, explicitEigenStrain=eigenstrain)
    solver = TrussSolver(truss)
    res = solver.solve()
    results.append(res)
